...
func (b *NetworkMapBuilder) buildPeerRoutesView(account *Account, peerID string) {
	...
	routeFirewallRules := b.getPeerRoutesFirewallRules(account, peerID, b.validatedPeers, peer.SupportsIPv6())
...
func (b *NetworkMapBuilder) addUpdateForPeersInGroups(
	...
	for _, groupID := range groupIDs {
		...
		for _, peerID := range peers {
			...
			if v6fr != nil && targetPeer.SupportsIPv6() {
...
func (b *NetworkMapBuilder) addUpdateForDirectPeerResource(
	...
	if newPeer.IPv6.IsValid() && targetPeer.SupportsIPv6() {
		v6fr := &FirewallRule{
			PolicyID:  rule.ID,
			PeerIP:    newPeer.IPv6.String(),
			Direction: direction,
